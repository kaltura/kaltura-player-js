...
describe('Dash Widevine Integration', function () {
  ...
  it('should play dash widevine drm on chrome', function (done) {
    ...
    kalturaPlayer.loadMedia(entryId).then(() => {
      if (DrmSupport.isProtocolSupported(DrmScheme.WIDEVINE, kalturaPlayer.config.sources.dash[0].drmData)) {
        kalturaPlayer.play();
      } else {
        done();
      }
