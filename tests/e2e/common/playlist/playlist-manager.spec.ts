...
describe('PlaylistManager', () => {
  ...
  describe('configure', () => {
    ...
    it('should load playlist by config and fire event', () => {
      kalturaPlayer._eventManager.listen(
        kalturaPlayer,
        kalturaPlayer.Event.Playlist.PLAYLIST_LOADED,
        (event) => {
    ...
    it('should fire playlist ended event', (done) => {
      ...
      kalturaPlayer._eventManager.listen(
        kalturaPlayer,
        kalturaPlayer.Event.Playlist.PLAYLIST_ENDED,
        () => {
          kalturaPlayer._eventManager.unlisten(
            kalturaPlayer,
            kalturaPlayer.Event.Playlist.PLAYLIST_ITEM_CHANGED,
            onItemChanged
          );
      ...
      kalturaPlayer.configure({
        sources: {
          options: {
            ...
            redirectExternalStreamsHandler: () => 1
          }
        }
      });
      playlistManager.configure({ items: [{ sources: {} }, { sources: {} }] });
      playlistManager.items.forEach(
        (item) =>
          item.sources.options.forceRedirectExternalStreams.should.be.true
      );
      playlistManager.items.forEach(
        (item) =>
          (item.sources.options.redirectExternalStreamsHandler() === 1).should
            .be.true
      );
    });
    ...
    it('should prefer the item configuration before the sources.options', () => {
      kalturaPlayer.configure({
        sources: {
          options: {
            forceRedirectExternalStreams: true,
            redirectExternalStreamsHandler: (): number => 1
          }
        }
      });
      playlistManager.configure({
        items: [
          ...
          {
            sources: {
              options: { redirectExternalStreamsHandler: (): number => 2 }
            }
          }
        ]
      });
      playlistManager.items[0].sources.options.forceRedirectExternalStreams
        .should.be.false;
      playlistManager.items[1].sources.options.forceRedirectExternalStreams
        .should.be.true;
      (
        playlistManager.items[0].sources.options.redirectExternalStreamsHandler() ===
        1
      ).should.be.true;
      (
        playlistManager.items[1].sources.options.redirectExternalStreamsHandler() ===
        2
      ).should.be.true;
    });
  });
  ...
  describe('load', () => {
    it('should merge the playlist data and config', () => {
      ...
      playlistManager.load(PlaylistMockData.playlistByID, playlistConfig, {
        entries: [{ entryId: '123' }]
      });
      ...
      playlistManager._playlist.poster.should.equal(
      ...
      playlistManager._playlist.items[0]._config.countdown.timeToShow.should.be.equal(
        40
      );
      playlistManager._playlist.items[0]._config.countdown.duration.should.be.equal(
        5
      );
      playlistManager._playlist.items[0]._config.countdown.showing.should.be
        .true;
      playlistManager._playlist.items[1]._sources.id.should.equal('0_15xrxwvo');
      playlistManager._playlist.items[1]._sources.hls[0].url.should.equal(
        'source_url'
      );
      playlistManager._playlist.items[1]._sources.poster.should.equal(
        'poster_url'
      );
  ...
  describe('provider plugins', () => {
    ...
    it("should apply the app's plugin config before the provider's and reset the provider's on change media", (done) => {
      kalturaPlayer._eventManager.listenOnce(
        kalturaPlayer,
        PlaylistEventType.PLAYLIST_ITEM_CHANGED,
        () => {
          try {
            kalturaPlayer.config.plugins.colors.favouriteColor.should.equals(
              'green'
            );
            kalturaPlayer.config.plugins.colors.someProp1.should.equals(
              'app_prop1'
            );
