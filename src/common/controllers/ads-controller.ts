...
import { Error, EventManager, AdEventType, FakeEvent, FakeEventTarget, Html5EventType, CustomEventType, getLogger, BaseMiddleware, Utils } from '@playkit-js/playkit-js';
...
class AdsController extends FakeEventTarget implements IAdsController {
  ...
  private _handleConfiguredAdBreaks(): void {
    ...
    this._configAdBreaks = this._player.config.advertising.adBreaks
      .filter((adBreak) => (typeof adBreak.every === 'number' || typeof adBreak.position === 'number' || typeof adBreak.percentage === 'number') && adBreak.ads.length)
      .map((adBreak) => {
  ...
  private _dispatchAdManifestLoaded(): void {
    const adBreaksPosition = Array.from(
      new Set(this._configAdBreaks.map((adBreak) => (adBreak.every && adBreak.every + 's') || (typeof adBreak.percentage === 'number' && adBreak.percentage + '%') || adBreak.position))
    );
  ...
  private _onEnded(): void {
    ...
    const bumperCompleted = bumperCtrl && typeof bumperCtrl.onPlaybackEnded === 'function' ? (): Promise<any> => bumperCtrl.onPlaybackEnded() : (): Promise<any> => Promise.resolve();
    const adCompleted = adCtrl && typeof adCtrl.onPlaybackEnded === 'function' ? (): Promise<any> => adCtrl.onPlaybackEnded() : (): Promise<any> => Promise.resolve();
