...
class ViewabilityManager {
  ...
  private _targetsObserved: Utils.MultiMap<
    HTMLElement,
    _TargetObserveredBinding
  >;
  ...
  constructor(viewabilityConfig: ViewabilityConfig = {}) {
    viewabilityConfig.observedThresholds =
      viewabilityConfig.observedThresholds || DEFAULT_OBSERVED_THRESHOLDS;
    viewabilityConfig.playerThreshold =
      typeof viewabilityConfig.playerThreshold === 'number'
        ? viewabilityConfig.playerThreshold
        : DEFAULT_PLAYER_THRESHOLD;

    this._config = viewabilityConfig;
    this._eventManager = new EventManager();
    this._targetsObserved = new Utils.MultiMap<
      HTMLElement,
      _TargetObserveredBinding
    >();
    const options = {
    ...
    this._observer = new window.IntersectionObserver(
      this._intersectionChangedHandler.bind(this),
      options
    );
  ...
  private _intersectionChangedHandler(
    ...
    entries.forEach((entry: IntersectionObserverEntry) => {
      const targetObserveredBindings: Array<_TargetObserveredBinding> =
        this._targetsObserved.get(entry.target as HTMLElement);
      targetObserveredBindings.forEach(
        (targetObservedBinding: _TargetObserveredBinding) => {
          const visible =
            entry.intersectionRatio >= targetObservedBinding.threshold;
  ...
  private _handleTabVisibilityChange(): void {
    ...
    this._targetsObserved
      .getAll()
      .forEach((targetObservedBinding: _TargetObserveredBinding) => {
        if (targetObservedBinding.lastVisible) {
          targetObservedBinding.listener(
            this._isTabVisible,
            ViewabilityType.TAB
          );
        }
  ...
  private _initTabVisibility(): void {
    ...
    if (this._visibilityTabHiddenAttr && this._visibilityTabChangeEventName) {
      this._eventManager.listen(
        document,
        this._visibilityTabChangeEventName,
        this._handleTabVisibilityChange.bind(this)
      );
      this._isTabVisible = !document[this._visibilityTabHiddenAttr];
    }
  }
  ...
  public observe(
    ...
    const threshold =
      typeof optionalThreshold === 'number'
        ? optionalThreshold
        : this._config.playerThreshold;
    ...
    if (!this._targetsObserved.has(target)) {
      ...
      const lastIntersectionRatio =
        this._targetsObserved.get(target)[0].lastIntersectionRatio;
      ...
      if (lastIntersectionRatio !== undefined) {
        ...
        newTargetObservedBinding.listener(
          this._isTabVisible &&
            lastIntersectionRatio >= newTargetObservedBinding.threshold,
  ...
  public unObserve(
    target: HTMLElement,
    listener: _TargetObserveredBinding
  ): void {
    if (!this._observer) return;
...
const DEFAULT_OBSERVED_THRESHOLDS: Array<number> = [
